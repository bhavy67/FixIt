...
export async function processPdfCrop(
  ctx: ProcessingContext<PdfCropOptions>,
  ...
  const input = { buffer, options };

  const bytes = await runInWorker<Uint8Array>({
    worker,
    input,
    transfer: [buffer],
    signal,
  ...
  return {
    outputs: [{ blob, filename: `${base}-cropped.pdf`, bytes: blob.size }],
  };
}
