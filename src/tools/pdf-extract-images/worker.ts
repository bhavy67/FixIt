...
function post(
  message: WorkerMessage<PdfExtractImagesWorkerResult>,
...
ctx.addEventListener('message', async (e: MessageEvent<PdfExtractImagesWorkerInput>) => {
  try {
    const { buffer, options, basename } = e.data;
    ...
    for (let pageNum = 1; pageNum <= pageCount; pageNum++) {
      ...
      for (let i = 0; i < opList.fnArray.length; i++) {
        ...
        imgIndex++;
        images.push({
          filename: `${basename}-img-${imgIndex}.${ext}`,
          bytes,
          mime,
        });
      }

      post({ type: 'progress', value: pageNum / pageCount });
    }

    if (images.length === 0) {
      post({
        type: 'error',
        message: 'No images found in this PDF. It may contain only text or vector graphics.',
      });
      return;
    }

    const transfer = images.map((im) => im.bytes.buffer as ArrayBuffer);
    post({ type: 'result', value: { images } }, transfer);
  } catch (err) {
